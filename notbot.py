...
# Mesajı Telegram limitlerine göre bölen yardımcı fonksiyon
async def mesaj_gonder_bolerek(update_or_context, chat_id, text):
    if len(text) <= 4096:
        ...
        for i in range(0, len(text), 4096):
            chunk = text[i:i+4096]
            if hasattr(update_or_context, 'message') and update_or_context.message:
                await update_or_context.message.reply_text(chunk, parse_mode="Markdown")
            else:
                await update_or_context.bot.send_message(chat_id=chat_id, text=chunk, parse_mode="Markdown")
...
def notlari_tara_fast(email, sifre):
    ...
    try:
        ...
        if not ders_select: return "❌ Giriş başarısız."
        ...
        for d_id, d_adi in dersler:
            ...
            for tablo in tablolar:
                if any(x in tablo.text for x in ["Vize", "Final", "Başarı", "Ara Sınav"]):
                    for row in tablo.find_all('tr'):
                        cols = row.find_all('td')
                        if len(cols) >= 5:
                            adi, notu = cols[0].text.strip(), cols[4].text.strip()
                            if any(x in adi for x in ["Vize", "Final", "Başarı", "Quiz", "Ara Sınav"]):
                                sonuc += f" - {adi}: `{notu if notu else 'Yok'}`\n"
                                ders_not_bulundu = True
                    if ders_not_bulundu: break
            if not ders_not_bulundu: sonuc += " - Not girişi henüz yok.\n"
...
async def manuel_kontrol(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if user:
        await u.message.reply_text("🔍 Notlar çekiliyor...")
...
async def otomatik_kontrol(c: ContextTypes.DEFAULT_TYPE):
    ...
    if user:
        res = notlari_tara_fast(user[0], user[1])
        await mesaj_gonder_bolerek(c, c.job.chat_id, f"🔔 *OTOMATİK KONTROL SONUCU:*\n{res}")
